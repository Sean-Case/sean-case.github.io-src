---
title: RECOUNT - Influence of Sequoia AVC Edge touchscreen use on the Wisconsin Trump vote
author: Sean Case
...
model.r.df = read.csv("modeldfward.r.csv",stringsAsFactors = F)
# model.r.df = subset(model.r.df, total_ballots > 1200)
...
model.r.df$demwinner2012 = with(model.r.df, ifelse(demwinner2012 == 0, "No", model.r.df$demwinner2012))

model.r.df$tch_prop_split = with(model.r.df, ifelse(tch_prop < 0.25, 0, 1))
model.r.df$turnout_highlow = with(model.r.df, ifelse(turnout_reg_feb <= 0.85, 0, 1))
model.r.df$density_levels = with(model.r.df, ifelse(muni.dense < 50, "< 50", 1))
...
# 
model.r.df$turnout_levels = with(model.r.df, ifelse(turnout_reg_feb < .80, "< 80", 1))
model.r.df$turnout_levels = with(model.r.df, ifelse(turnout_reg_feb >= .80, "80 - 89", model.r.df$turnout_levels))
model.r.df$turnout_levels = with(model.r.df, ifelse(turnout_reg_feb >= .90, "90 - 94", model.r.df$turnout_levels))
model.r.df$turnout_levels = with(model.r.df, ifelse(turnout_reg_feb >= .95, "95 +", model.r.df$turnout_levels))

model.r.df$above_mean_tch = with(model.r.df, ifelse(republican_votes.perc > 
                                  mean(republican_votes.perc[tch_prop == 0],na.rm=T), "Greater", "Less"))

model.r.df$inc_levels = NA
model.r.df$inc_levels = with(model.r.df, ifelse(inc_2015 < median(inc_2015,na.rm=T), "Below median", model.r.df$inc_levels))
model.r.df$inc_levels = with(model.r.df, ifelse(inc_2015 >= median(inc_2015,na.rm=T), "Higher than median", model.r.df$inc_levels))

model.r.df$grad_levels = NA
model.r.df$grad_levels = with(model.r.df, ifelse(pct_graduate < median(pct_graduate,na.rm=T), "Below median", model.r.df$grad_levels))
model.r.df$grad_levels = with(model.r.df, ifelse(pct_graduate >= median(pct_graduate,na.rm=T), "Higher than median", model.r.df$grad_levels))

model.r.df$most_likely_places = with(model.r.df, ifelse(muni.dense > 200 & tch_prop_split == 1 & tch_model_off_full == "Dominion (Sequoia)/Command Central-Edge",1,0))
  ...
  # remove "Dominion (Premier)-Accuvote TSX"
    ...
    model.r.df$turnout_reg_feb[model.r.df$turnout_reg_feb > 1] = 1; model.r.df$tch_prop = ifelse(model.r.df$tch_prop > 1 & model.r.df$tch_model_off_full == "aNone",NA,model.r.df$tch_prop)
     # model.r.df$turnout_reg_feb[model.r.df$turnout_reg_feb < 0.6] = NA
    ...
    model.r.df$trump_vote_change = model.r.df$republican_votes.perc - model.r.df$votes_perc_trump
    
    model.r.df$tch_prop_split = with(model.r.df, ifelse(tch_prop_split == 1 , "Yes", model.r.df$tch_prop_split))
    model.r.df$tch_prop_split = with(model.r.df, ifelse(tch_prop_split == 0, "No", model.r.df$tch_prop_split))
...
model.r.df$turnout_levels = as.factor(model.r.df$turnout_levels)
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(demwinner2012, density_levels,tch_prop_split,rep.change.perc)))
...
model.r.df$turnout_levels = as.factor(model.r.df$turnout_levels)
          ...
          dem.tchuse.density.percvote = ggplot(data = model.r.df[!is.na(model.r.df$tch_prop_2012)& !is.na(model.r.df$demwinner2012),],
                                                      aes(x = tch_prop_2012,
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop_demprim) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(demwinner2012,tch_prop_split,rep.perc2012)))
    tch.dem.change.cast.simple = dcast(tch.dem.change.melt, tch_prop_split ~variable, mean,na.rm=T)
...
# tch.demwin.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop_2012) & !is.na(model.r.df$density_levels) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(tch_prop_split,demwinner2012,density_levels,dem.perc2012)))
...
model_formula = formula(rep.perc2012 ~ total_ballots_2012 + #+ os_use_off
  ...
  # ols1 = lm(model_formula, data = model.r.df, na.action = na.exclude)
   # summary(ols1)
  # AIC(ols1)

ols2 <- ols(model_formula, data=model.r.df, x=TRUE, y=TRUE)
# print(ols2)
...
model.r.df$turnout_levels = as.factor(model.r.df$turnout_levels)
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(demwinner2012, density_levels,tch_prop_split,republican_votes.perc)))
    tch.dem.change.cast.simple = dcast(tch.dem.change.melt, tch_prop_split ~variable, mean,na.rm=T)
  colnames(tch.dem.change.cast.simple) = c("AVC Edge machines used by > 25% of voters?","Republican vote (%)")
  kable(tch.dem.change.cast.simple,digits = 1, format = "markdown",padding = 0, align = "c")
...
# tch.demwin.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop) & !is.na(model.r.df$density_levels) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(tch_prop_split,demwinner2012,density_levels,republican_votes.perc)))
#     tch.demwin.dem.change.cast = dcast(tch.demwin.dem.change.melt, density_levels+tch_prop_split~demwinner2012+variable, mean,na.rm=T)
...
ols22 <- ols(model_formula2, data=model.r.df, x=TRUE, y=TRUE)
# print(ols2)
# Second, the cluster-robust covariance is called
cov <- robcov(ols22, cluster=model.r.df$county)
print(cov)
# print(cov$var)
# AIC(cov)

# cor(model.r.df$rep.change.perc, model.r.df$republican_votes.perc,method="pearson")
```
...
model.r.df$turnout_levels = as.factor(model.r.df$turnout_levels)
          ...
          dem.tchuse.density.percvote = ggplot(data = model.r.df[                 !is.na(model.r.df$tch_prop_reprim)& !is.na(model.r.df$demwinner2012),],
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop_reprim) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(demwinner2012,tch_prop_split,votes_perc_trump_reprim)))
    tch.dem.change.cast.simple = dcast(tch.dem.change.melt, tch_prop_split ~variable, mean,na.rm=T)
...
# tch.demwin.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop_reprim) & !is.na(model.r.df$density_levels) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(tch_prop_split,demwinner2012,density_levels,votes_perc_trump_reprim)))
#     tch.demwin.dem.change.cast = dcast(tch.demwin.dem.change.melt, density_levels+tch_prop_split~demwinner2012+variable, mean,na.rm=T)
#     colnames(tch.demwin.dem.change.cast) = c("Population density (ppl / km2)","AVC Edge machines used by > 25% of voters?","Vote Republicans 2016 (%) - areas Dem lost 2012","Vote Republicans 2016 (%) - areas Dem won 2012")
# kable(tch.demwin.dem.change.cast,digits = 1, format = "markdown",padding = 0, align = "c")
```
...
model.r.df$turnout_levels = as.factor(model.r.df$turnout_levels)
          ...
          dem.tchuse.density.percvote = ggplot(data = model.r.df[!is.na(model.r.df$demwinner2012) &                  !is.na(model.r.df$tch_prop)& !is.na(model.r.df$demwinner2012),],
                                                      aes(x = tch_prop,
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop_reprim) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(demwinner2012,tch_prop_split,trump_vote_change)))
    tch.dem.change.cast.simple = dcast(tch.dem.change.melt, tch_prop_split ~variable, mean,na.rm=T)
...
# tch.demwin.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$demwinner2012) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(tch_prop_split,density_levels,demwinner2012,trump_vote_change)))
#     tch.demwin.dem.change.cast = dcast(tch.demwin.dem.change.melt, density_levels+tch_prop_split~demwinner2012+variable, mean,na.rm=T)
...
model.r.df$inc_levels = as.factor(model.r.df$inc_levels)
model.r.df$tch_prop_split = as.factor(model.r.df$tch_prop_split)
model.r.df$density_levels = as.factor(model.r.df$density_levels)

          dem.tchuse.density.percvote = ggplot(data = model.r.df[                 !is.na(model.r.df$tch_prop)& !is.na(model.r.df$inc_levels),],
                                                      aes(x = tch_prop,
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop)  & !is.na(model.r.df$inc_levels) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(inc_levels, density_levels,tch_prop_split,rep.change.perc)))
...
model.r.df$grad_levels = as.factor(model.r.df$grad_levels)
model.r.df$tch_prop_split = as.factor(model.r.df$tch_prop_split)
model.r.df$density_levels = as.factor(model.r.df$density_levels)

          dem.tchuse.density.percvote = ggplot(data = model.r.df[                 !is.na(model.r.df$tch_prop)& !is.na(model.r.df$grad_levels),],
                                                      aes(x = tch_prop,
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop) & !is.na(model.r.df$grad_levels) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(grad_levels, density_levels,tch_prop_split,rep.change.perc)))
...
model.r.df$density_levels = as.factor(model.r.df$density_levels)
          ...
          dem.tchuse.density.percvote = ggplot(data = model.r.df[!is.na(model.r.df$density_levels) &                  !is.na(model.r.df$tch_prop)& !is.na(model.r.df$density_levels),],
                                                      aes(x = tch_prop,
  ...
  tch.dem.change.melt = with(model.r.df[!is.na(model.r.df$tch_prop) & !is.na(model.r.df$density_levels) & !is.na(model.r.df$tch_prop_split),], melt(data.frame(density_levels, density_levels,tch_prop_split,rep.change.perc)))
